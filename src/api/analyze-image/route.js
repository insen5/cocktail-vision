export default async function handler(req) {
  try {
    ...
    if (!imageBase64) {
      ...
      return new Response(
        JSON.stringify({ error: "No image provided", body: { allDetected: [] } }),
    ...
    if (!success) {
      ...
      return new Response(
        JSON.stringify({ 
          error: "Failed to analyze image with available APIs", 
          body: { allDetected: [] } 
        }),
    ...
    return new Response(
      JSON.stringify({ error: "Failed to analyze image", body: { allDetected: [] } }),
      { status: 500, headers: { "Content-Type": "application/json" } }
    );
  }
}
